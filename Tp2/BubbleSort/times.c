...
void insertionSorting(long int listaOrdenada, int movimentacoes, int comparacoes, int pos){
	if(pos > 0){
		insertionSorting(listaOrdenada, movimentacoes, comparacoes, pos-1);	
	}
} 
...
void ORQUESTRADOR(char entrada[], long int* ordenar){
...
void SORT(char entrada[], int numArquivo)
{
	FILE *arq;
	...
	long int lista[TAM];
	ORQUESTRADOR(entrada, &lista[numArquivo]);
	numArquivo++;
	...
	clock_t final = clock();
	double total = (final - inicio) / CLOCKS_PER_SEC;

	//Numero de comparações (do array)
	fprintf(arq, "%s\t", "COMPARACOES");

	//Numero de movimentações (do array)
	fprintf(arq, "%s\t", "MOVIMENTACOES");
